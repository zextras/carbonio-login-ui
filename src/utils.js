...
export function deviceId() {
  ...
  const generatedUUID = ([1e7] + -1e3 + -4e3 + -8e3 + -1e11).replace(
    /[018]/g,
    (c) =>
      (
...
export function generateColorSet(
  { regular, hover, active, disabled, focus },
  dark = false
) {
  return {
    regular,
    ...
    disabled:
      disabled ?? dark
        ? setLightness(0.8, regular)
        : desaturate(0.3, darken(0.3, regular)),
  };
}
...
export const setCookie = (cName, cValue, expDays) => {
  ...
  const expires =
    expDays && Number.isInteger(expDays)
