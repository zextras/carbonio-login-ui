		...
		<form onSubmit={handleSubmit} style={{ width: '100%' }}>
			...
			>
				<Button
					...
					label={t('login.twoFactorAuthentication.otpSetup.backToLoginPage', 'Back to login page')}
			...
			<Row padding={{ bottom: 'small', top: 'large' }} mainAlignment="flex-start">
				<Text weight="bold" overflow="break-word">
					{t('login.twoFactorAuthentication.otpSetup.step1.title', 'Step 1')}
				</Text>
			</Row>
			<Row padding={{ bottom: 'large' }} mainAlignment="flex-start">
				<Text color="text" overflow="break-word">
					{t(
						'login.twoFactorAuthentication.otpSetup.step1.scanQr',
			...
			<Row padding={{ bottom: 'small', top: 'large' }} mainAlignment="flex-start">
				<Text weight="bold" overflow="break-word">
					{t('login.twoFactorAuthentication.otpSetup.step2.title', 'Step 2')}
				</Text>
			</Row>
			<Row padding={{ bottom: 'small' }} mainAlignment="flex-start">
				<Text color="text" overflow="break-word">
					{t(
						'login.twoFactorAuthentication.otpSetup.step2.enterCode',
			...
			<Row padding={{ bottom: 'large' }}>
				<Input
					...
					label={t(
						'login.twoFactorAuthentication.otpSetup.codePlaceholder',
						'Insert the 6-digit code generated'
					)}
			...
			<Row padding={{ bottom: 'extrasmall' }} mainAlignment="flex-start">
				<Text color="error" size="small" overflow="break-word">
					{hasError
						? t(
								'login.twoFactorAuthentication.otpSetup.codeError',
			...
			<Row padding={{ bottom: 'small' }} mainAlignment="flex-start">
				<Checkbox
					value={trustDevice}
					label={t(
						'login.twoFactorAuthentication.otpSetup.rememberDeviceOrIp',
						'Remember this device or IP'
					)}
			...
			<Row mainAlignment="space-between" padding={{ vertical: 'small' }} width="100%">
				<Row padding={{ all: 'small' }} width="50%">
					<Button
						type="outlined"
						onClick={onBack}
						label={t('login.twoFactorAuthentication.otpSetup.back', 'Back')}
						style={{ flex: 1 }}
				...
				<Row padding={{ all: 'small' }} width="50%">
					<Button
						...
						label={t(
							'login.twoFactorAuthentication.otpSetup.verifyAndProceed',
							'Verify and proceed'
						)}
						loading={loading}
						style={{ flex: 1 }}
