...
function ModifiedTheme({ changes }) {
  const proxyFn = useCallback(
    (draft, variant) => modifyTheme(draft, variant, changes),
    []
  );
...
const LoginContainer = styled(Container)`
  padding: 0 100px;
  background: url(${(props) => props.backgroundImage}) no-repeat 75%
    center/cover;
  ...
  useLayoutEffect(() => {
    ...
    if (hasBackendApi) {
      getLoginConfig(version, domain, domain)
        .then((res) => {
          if (!destinationUrl)
            setDestinationUrl(prepareUrlForForward(res.publicUrl));
