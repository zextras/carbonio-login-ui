...
import {
	Button, Container, Link, Padding, Row, Snackbar, Text, Tooltip, useScreenMode
...
export default function LoginView({ theme, setTheme }) {
	...
	useEffect(() => {
		...
		getAuthSupported(domain)
			...
			.catch(() => {
				if (componentIsMounted) setError();
			});
	...
	}, [destinationUrl, domain, retryConfig, setError, setTheme]);
	...
	return (
		...
		src={logo.image} style={{
                											maxWidth: '60%', maxHeight: '150px', display: 'block', marginLeft: 'auto', marginRight: 'auto'
                									}}
	/>
 </a>
 </Container>
	...
	<Button
	type="outlined" label={t('Login SAML')} color="primary"
...
Zextras,
	{' '}
                		...
                		<Snackbar
                			open={snackbarThemeError} onClose={() => setSnackbarThemeError(false)} type="error"
